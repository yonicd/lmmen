...
<<echo=FALSE>>=
library(ggplot2);library(scales);library(dplyr);library(grid);library(ggthemes)
load(file="sim.output/graph_data2.rdata")
...
temp.compare$simtype=factor(temp.compare$simtype,labels=c("LMMEN",'LMMLASSO',"Pen.LME"))
...
p.compare= temp.compare%>%filter(!is.na(pct))%>%mutate(ex=factor(ex))%>%
  ...
  scale_fill_grey(name="Simulation \n Type")+theme(legend.position="bottom")+
...
p.case=ggplot(temp.case%>%filter(id!=36),aes(x=factor(id),y=mest,fill=pct))+geom_bar(stat="identity",position="dodge")+facet_grid(simtype~type,scales="free_x")+theme_bw()+scale_fill_grey(name="Persistency",breaks=seq(0,1,0.1),labels=percent)+scale_x_discrete(labels=seq(1:39))+ylab("Mean Parameter Estimate")+xlab("Parameter")

gen.plot=fit.min.all%>%mutate(id=ifelse(id>20,21,id))%>%filter(type!="BIC")
gen.plot$type=factor(gen.plot$type,labels=c("Fixed Effects","Random Effects"))
gen.plot$simtype=factor(gen.plot$simtype,labels=c("LMMEN",'LMMLASSO',"Pen.LME"))

p4=gen.plot%>%mutate(id=factor(id,labels=c(1:20,"21+")),flabel=sprintf('%s: %s',ex,type))%>%plyr::dlply(c('ex'),.fun=function(df){
  df%>%ggplot(aes(x=id,y=est,fill=simtype))+geom_boxplot()+facet_wrap(~flabel,scales="free",ncol=2)+xlab("Parameter")+ylab("Estimate")+scale_fill_discrete(name="Simulation Type")+theme_bw(base_size = fs)+theme(legend.position="bottom")
})
...
p_eps_r=ggplot(case.panel$ran,aes(x=factor(var),y=stat,fill=Type))+geom_bar(stat = "identity")+ylab('% Persistence')+xlab('Random Effect')+theme_bw(base_size = fs)+
  facet_wrap(~Type)+scale_y_continuous(limits=c(0,1),labels=percent)+  scale_fill_grey(guide=FALSE)
