...
initialize_example <- function(n.i = 5,n = 30,q=4,total.beta=9,
  ...
  bvec <- c(9,4.8,0.6,0, 4.8,4,1,0, .6,1,1,0, 0,0,0,0)
  if (q<10) {
    true.D = matrix(bvec, nrow=q, ncol=q)
  } else {
    true.D = matrix(bvec, nrow=4, ncol=4)
  ...
  for (i in 1:n) { 
    ...
    m <- data.frame(subject=factor(subject), m)
    return(m)
}
